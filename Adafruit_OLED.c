...
static long spiTransferByte(unsigned char c)
{
    ...
    ulDummy = HWREG(GSPI_BASE + MCSPI_O_RX0);
    (void)ulDummy;
...
void writeCommand(unsigned char c) {
    ...
    GPIOPinWrite(GPIOA0_BASE, 0x40, 0);     // Set DC LOW (command mode)
    GPIOPinWrite(GPIOA0_BASE, 0x80, 0);     // Set OC LOW  (CS assert)
    MAP_SPICSEnable(GSPI_BASE);             // Start GSPI transfer in SW CS mode

    if (spiTransferByte(c) != 0) {
        Report("SPI stalled while sending command 0x%02x\n\r", c);
    }

    MAP_SPICSDisable(GSPI_BASE);            // Stop GSPI transfer
    GPIOPinWrite(GPIOA0_BASE, 0x80, 0x80);  // Set OC HIGH (CS deassert)
...
void writeData(unsigned char c) {
    ...
    GPIOPinWrite(GPIOA0_BASE, 0x40, 0x40);  // Set DC HIGH (data mode)
    GPIOPinWrite(GPIOA0_BASE, 0x80, 0);     // Set OC LOW  (CS assert)
    MAP_SPICSEnable(GSPI_BASE);             // Start GSPI transfer in SW CS mode

    if (spiTransferByte(c) != 0) {
        Report("SPI stalled while sending data 0x%02x\n\r", c);
    }
...
*/
  ...
  GPIOPinWrite(GPIOA3_BASE, 0x10, 0x10);  // Start with RESET HIGH
  MAP_UtilsDelay(800000);                 // ~10ms at 80MHz

  GPIOPinWrite(GPIOA3_BASE, 0x10, 0);    // RESET = RESET_LOW
  MAP_UtilsDelay(800000);                 // Hold low for ~10ms (min 100ns needed)

  GPIOPinWrite(GPIOA3_BASE, 0x10, 0x10); // RESET = RESET_HIGH
  MAP_UtilsDelay(8000000);               // Wait ~100ms for display to finish reset

  Report("OLED reset complete, starting SSD1351 init\n\r");
...
//    writeData(0x32);
  ...
  MAP_UtilsDelay(8000000);               // Wait ~100ms for display to turn on
  Report("OLED init sequence complete\n\r");
}
